#!/usr/bin/env python3
"""
Verify Context Parallel (CP) works correctly WITH document masking.

This test:
1. Creates a small transformer model
2. Runs forward pass WITHOUT CP + document masking (baseline)
3. Runs forward pass WITH CP + document masking (using manual all-gather)
4. Compares outputs - they should be identical

Run with 2 GPUs:
  torchrun --nproc_per_node=2 debug_cp_document_masking.py
"""
...
def _get_document_ids(seq_lens, device):
    """Create document IDs tensor from sequence lengths."""
    batch_document_ids = []
...
def get_document_causal_mask_mod(document_ids):
    """Create a mask_mod that enforces causal + same-document constraints."""
...
def main():
    ...
    log(rank, "=" * 60)
    log(rank, f"Testing CP correctness WITH DOCUMENT MASKING")
    ...
    # Document structure: 4 documents of 64 tokens each
    # This means with CP=2, each rank gets 2 complete documents
    ...
    log(
        rank,
        f"Rank {rank} handles positions {rank*local_seq_len} to {(rank+1)*local_seq_len-1}",
    )

    # Create document IDs (same on all ranks - full sequence)
    seq_lens_batch = [[s for s in doc_sizes] for _ in range(batch_size)]
    document_ids = _get_document_ids(seq_lens_batch, device)

    log(
        rank,
        f"Document ID range for this rank: {document_ids[0, rank*local_seq_len].item()} to {document_ids[0, (rank+1)*local_seq_len-1].item()}",
    )
    ...
    # ========================================
    # Test 1: Full sequence forward (baseline with document masking)
    # ========================================
    log(
        rank, "\n--- Test 1: Full sequence forward WITH document masking (baseline) ---"
    )

    # Create full document-causal mask
    doc_mask_mod = get_document_causal_mask_mod(document_ids)
    ...
    # ========================================
    # Test 2: CP forward with document masking
    # ========================================
    log(rank, "\n--- Test 2: CP forward WITH document masking ---")
    ...
    # Create CP-aware document mask
    # Q is local (0 to local_seq_len-1), KV is global (0 to seq_len-1)
    # We need to offset q_idx to get correct document lookup
    ...
    if rank == 0:
        log(rank, "\n" + "=" * 60)
        log(rank, "CP DOCUMENT MASKING TEST COMPLETE")
        log(rank, "=" * 60)
