...
def convert_llama_weights(llama_model, output_dir):
    hf_model = AutoModelForCausalLM.from_pretrained(llama_model, torch_dtype=torch.bfloat16)
        ...
        for wn, hn, nh in [
            ...
            if wn != "wv":
                ...
                state_dict[f"layers.{layer}.attention.{wn}.weight"] = reverse_permute(
                    ...
                    dim1=dim if wn == "wq" else key_value_dim,
                    dim2=dim,
                )
                if f"model.layers.{layer}.self_attn.{hn}.bias" in hf_state_dict:
                    # bias is used in Qwen
                    state_dict[f"layers.{layer}.attention.{wn}.bias"] = (
                        reverse_permute_1d(
