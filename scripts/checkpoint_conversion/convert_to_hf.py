...
def convert_to_hf(input_dir, output_dir, model_name, model_flavor, hf_assets_path, dtype=torch.bfloat16):
    if model_name == "flux":
...
def convert_all_checkpoints(
    checkpoint_dir: Path, model_name: str, model_flavor: str, hf_assets_path: Path, dtype=torch.bfloat16
):
    ...
    for step_dir in step_dirs:
        ...
        convert_to_hf(step_dir, output_dir, model_name, model_flavor, hf_assets_path, dtype)
        print(f"  -> {output_dir}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Export dtype for weights (default: bfloat16)"
    )
