...
class QwenVisionTransformer(nn.Module):
    ...
    def fast_pos_embed_interpolate(self, grid_thw):
        ...
        merge_size = self.args.spatial_merge_size
    ...
    def forward(self, hidden_states: torch.Tensor, pixel_masks_NL: torch.BoolTensor,  grid_thw: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        pos_embeds = self.fast_pos_embed_interpolate( grid_thw.cuda() )
        #pad_rows = 4096 - pos_embeds.shape[0]
        # pad up to max_seq_len
        #pos_embeds = F.pad(pos_embeds, (0, 0, 0, pad_rows))  # pad only along dim 0, at the end

        hidden_states[0,:pos_embeds.shape[0]] = hidden_states[0,:pos_embeds.shape[0]] + pos_embeds

        #idden_states = hidden_states[0,:pos_embeds.shape[1]] + pos_embeds
