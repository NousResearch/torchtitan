...
def apply_fsdp(
    ...
    """
    # Log detailed FSDP sharding information
    logger.info(f"[FSDP-DEBUG] ========== FSDP SHARDING CONFIGURATION ==========")
    logger.info(f"[FSDP-DEBUG] dp_mesh: {dp_mesh}")
    logger.info(f"[FSDP-DEBUG] dp_mesh.size(): {dp_mesh.size()}")
    logger.info(f"[FSDP-DEBUG] dp_mesh.mesh_dim_names: {dp_mesh.mesh_dim_names}")
    if dp_mod_ep_mesh is not None:
        logger.info(f"[FSDP-DEBUG] dp_mod_ep_mesh: {dp_mod_ep_mesh}")
        logger.info(f"[FSDP-DEBUG] dp_mod_ep_mesh.size(): {dp_mod_ep_mesh.size()}")
        logger.info(
            f"[FSDP-DEBUG] dp_mod_ep_mesh.mesh_dim_names: {dp_mod_ep_mesh.mesh_dim_names}"
        )
    logger.info(f"[FSDP-DEBUG] ep_degree: {ep_degree}")
    logger.info(f"[FSDP-DEBUG] cpu_offload: {cpu_offload}")
    logger.info(
        f"[FSDP-DEBUG] reshard_after_forward_policy: {reshard_after_forward_policy}"
    )
    logger.info(f"[FSDP-DEBUG] =================================================")
    ...
    for layer_id, transformer_block in model.layers.items():
        ...
        if transformer_block.moe_enabled and ep_degree > 1:
            ...
            # FIX 2: Skip FSDP wrapping for expert params entirely
            # Experts use EP for parallelism, FSDP wrapping causes buffer scaling issues
            # See: https://github.com/pytorch/pytorch/issues/147179
            # COMMENTED OUT:
            # fully_shard(
            ...
            # )
            logger.info(
                f"[FSDP-DEBUG] Skipping FSDP wrapping for experts (layer {layer_id})"
            )

            # Log expert sharding info for the first layer only
            if layer_id == "0" or layer_id == 0:
                expert_params = sum(
                    p.numel() for p in transformer_block.moe.experts.parameters()
                )
                logger.info(
                    f"[FSDP-DEBUG] Expert params (layer 0) after FSDP shard: {expert_params:,} elements"
                )
                for name, p in transformer_block.moe.experts.named_parameters():
                    if "w1" in name or "w2" in name:
                        logger.info(
                            f"[FSDP-DEBUG]   {name}: shape={list(p.shape)}, numel={p.numel():,}"
                        )
                        break
            ...
            # COMMENTED OUT for FIX 2 (no FSDP wrapping for experts):
        ...
        # Log non-expert sharding info for the first layer only
        if layer_id == "0" or layer_id == 0:
            block_params = sum(p.numel() for p in transformer_block.parameters())
            logger.info(
                f"[FSDP-DEBUG] TransformerBlock (layer 0) total params after FSDP: {block_params:,} elements"
            )
            # Log attention params
            if hasattr(transformer_block, "attention"):
                attn_params = sum(
                    p.numel() for p in transformer_block.attention.parameters()
                )
                logger.info(
                    f"[FSDP-DEBUG]   Attention params: {attn_params:,} elements"
                )
                for name, p in transformer_block.attention.named_parameters():
    ...
    # Log final FSDP summary
    total_params = sum(p.numel() for p in model.parameters())
    logger.info(f"[FSDP-DEBUG] ========== FSDP SHARDING SUMMARY ==========")
    logger.info(
        f"[FSDP-DEBUG] Total model params after FSDP sharding: {total_params:,} elements"
    )
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated() / 1e9
        reserved = torch.cuda.memory_reserved() / 1e9
        logger.info(
            f"[FSDP-DEBUG] GPU Memory after FSDP: allocated={allocated:.2f} GiB, reserved={reserved:.2f} GiB"
        )
    logger.info(f"[FSDP-DEBUG] =============================================")
