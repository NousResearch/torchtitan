...
from torchtitan.models.nemotron_super.model.state_dict_adapter import (
    NemotronSuperStateDictAdapter,
)
...
HF_CKPT = str(
    Path.home()
    / ".cache/huggingface/hub/models--NousResearch--nns3_downsized/snapshots/f2a4b84e49d7bed795b501ab02e12b0e2517fdf6"
)
...
def main():
    ...
    for block_id, layer in model.layers.items():
        hooks.append(
            layer.mamba_norm.register_forward_hook(hook(f"b{block_id}_mamba_norm"))
        )
        hooks.append(layer.mamba.register_forward_hook(hook(f"b{block_id}_mamba")))
        if layer.has_attn:
            hooks.append(
                layer.attn_norm.register_forward_hook(hook(f"b{block_id}_attn_norm"))
            )
            hooks.append(
                layer.attention.register_forward_hook(hook(f"b{block_id}_attn"))
            )
        hooks.append(
            layer.ffn_norm.register_forward_hook(hook(f"b{block_id}_ffn_norm"))
        )
    ...
    match = (
        (hf_logits.argmax(-1) == our_logits.float().argmax(-1)).float().mean().item()
    )
    print(f"\n=== Logits ===")
    ...
    for flat_idx, label, our_key, hf_tensor in checks:
        ...
        if our_key is not None:
            ...
            if "mamba" in label:
                ...
                ours = (
                    outs[f"b{block}_mamba_norm_in"].float()
                    + outs[f"b{block}_mamba_out"].float()
                )
            elif "attn" in label:
                ...
                ours = (
                    outs[f"b{block}_ffn_norm_in"].float()
                    + outs[f"b{block}_moe_out"].float()
                )
