...
class PreprocessedMultimodalDataset(IterableDataset, Stateful):
    def __init__(
        self,
        dataset_name: str,
        dataset_path: str | None,
...
def collate_fn_multimodal(batch):
    inputs, labels, images = zip(*batch)

    expected_len = len(inputs[0]["input"])
    for i, (input_item, label_item, image_item) in enumerate(batch):
        input_len = len(input_item["input"])
        label_len = len(label_item)
    ...
    args = {"input": torch.stack([x["input"] for x in inputs]), "images": images}
    ...
    images_list = images 
    #images_list = [x["image"][0] for x in images]
...
def build_preprocessed_multimodal_dataloader(
    dp_world_size: int,
    dp_rank: int,
    tokenizer: Tokenizer,
    job_config: JobConfig,
    ...
    ds = PreprocessedMultimodalDataset(
        dataset_name=dataset_name,
        dataset_path=dataset_path,
        tokenizer=tokenizer,
        seq_len=seq_len,
