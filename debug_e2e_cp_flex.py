#!/usr/bin/env python3
"""
End-to-end training test with FlexAttention + CP.

Trains a small model for a few steps and compares:
1. Training WITHOUT CP (baseline)
2. Training WITH CP

Losses should match closely.

Run baseline (single GPU):
  python debug_e2e_cp_flex.py --no-cp

Run with CP (2 GPUs):
  torchrun --nproc_per_node=2 debug_e2e_cp_flex.py --cp
"""
...
def main():
    ...
    if args.cp:
        ...
        print("Usage:")
        print("  Baseline (no CP):  python debug_e2e_cp_flex.py --no-cp")
        print(
            "  With CP:           torchrun --nproc_per_node=2 debug_e2e_cp_flex.py --cp"
        )
        sys.exit(1)
